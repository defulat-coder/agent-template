...
test("fails the whole generation instead of skipping failed pages", () => {
  assert.deepEqual(zreadGenerateArguments, [
    "generate",
    "--draft",
    "clear",
    "--yes",
  ...
  assert.equal(zreadGenerateArguments.includes("--skip-failed"), false);
});
