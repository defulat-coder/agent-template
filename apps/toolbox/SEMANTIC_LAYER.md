...
| ---------------------------------- | ---------------------------------- | ------------------------ | ------: |
| `ecommerce-sales-analytics`        | `ecommerce-sales-analysis`         | 销售趋势、退款与渠道对比 |       2 |
| `ecommerce-product-analytics`      | `ecommerce-product-analysis`       | 商品排行与选品分析       |       1 |
...
`agent_template_read_model` 是为现有 Host 和部署配置保留的兼容 Toolset，不作为新的业务分组示例。四个业务 Toolset 都有官方 `skills-generate` caller；为了让 Skill 专注单一任务并减少 context rot，有意保持 1–2 个 Tool。在实际 Google Client SDK caller 出现前，不声明仅供文档展示的大 Toolset。

## MCP annotations

当前 15 个 SQL Tool 都是本地数据库内的确定性只读查询，统一声明：
