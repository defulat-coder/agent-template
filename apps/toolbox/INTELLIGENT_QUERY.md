...
Toolset 是 Skill 生成和业务分组，不是运行时授权。`AGENT_CAPABILITY_PROFILE` 通过 Claude SDK `allowedTools`/Tool policy 与 Eve connection `tools.allow` 收窄模型可见 Tool，但真正的授权 interface 是 Toolbox OIDC、Tool scope、数据库权限与 RLS/等效控制。不能把 Toolset 或模型可见范围描述成数据库最小权限控制。
...
6. **完整投影与验证**：同步更新 `tools.yaml`、Toolset、共享 Tool/Profile 配置、Claude/Eve 原生 MCP Client、原始与实际 Skills、语义目录和 golden cases；本地完成语义门禁、生成校验和 native Tool 执行验收。默认不使用 Docker；只有用户明确要求容器集成验证时才运行 Docker 门禁。
