...
- `AgentRunSnapshot.events` 使用持久化 envelope（sequence、executionAttempt、createdAt、event）；实时 `AgentRunResult.events` 保持裸 runtime event。
- Tool event 必须同时携带相关联的 `callId` 与 `toolName`；terminal result 使用 status discriminated union 强制字段不变量。
...
- ZRead 原生 `current`、`wiki.json`、源码路径与行号引用的 schema 和纯解析规则放这里；生成 adapter 与 Web 必须消费同一 interface，不向原生 Wiki 目录注入项目自定义 index。
