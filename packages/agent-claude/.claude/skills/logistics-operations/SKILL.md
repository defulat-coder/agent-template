...
2. 趋势问题先调用 `mcp__toolbox__summarize_carrier_performance` 或 `mcp__toolbox__summarize_delivery_sla`。
3. 异常排查调用 `mcp__toolbox__list_logistics_exceptions`，具体包裹再调用 `mcp__toolbox__get_shipment_trace`。
4. 成本问题调用 `mcp__toolbox__summarize_freight_costs`，不要从运费推断完整订单利润。
5. 明确说明 SLA、异常状态和参考时间，不把模拟数据描述成实时物流状态。

## Business semantic catalog

涉及业务术语、指标、维度或枚举取值时，先读取 `references/logistics.yaml`。只使用其中认证的术语、口径和 Tool；遇到标记为 `clarify` 的术语，先向用户澄清，不要猜测或生成任意 SQL。

## Available Toolbox tools

### `mcp__toolbox__get_shipment_trace`
...
### `mcp__toolbox__list_logistics_exceptions`
...
### `mcp__toolbox__summarize_carrier_performance`
...
### `mcp__toolbox__summarize_delivery_sla`
...
### `mcp__toolbox__summarize_freight_costs`
